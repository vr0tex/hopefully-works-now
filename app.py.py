...
GAME_ROLE_MAP = {
    "AG": 1500199147859476604,
    "ALS": 1500199051952656578,
    "AV": 1500198955940712468,
    "SP": 1500198307257913544,
    "AE": 1541834030717075457,
}

for required_asset in ("ag.png", "als.png", "av.png", "sp.png", "G2QVvk8O_400x400.jpg"):
    if not get_asset_path(required_asset):
        print(f"WARNING: Missing asset: {required_asset}")
...
def get_bonus_vouches(user_id):
    try:
        with open("bonus_vouches.json", "r") as f:
            data = json.load(f)
...
def create_vouch_embed(customer, booster, game, feedback, total_vouches, ticket_id, star_rating=5):
...
def add_bonus_vouches(user_id, amount, game):
    ...
    user_data = data.get(str(user_id), {"total": 0, "games": {}})
    user_data["total"] += amount
    user_data["games"][game] = user_data["games"].get(game, 0) + amount
    data[str(user_id)] = user_data
    
    with open("bonus_vouches.json", "w") as f:
        json.dump(data, f)
...
class VouchModal(discord.ui.Modal, title="Vouch Feedback"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Increment bonus vouches by 1 for every submission
            ...
            if vouch_channel:
                ...
                if supabase:
                    try:
                        ...
                        print(f"Error querying Supabase in VouchModal: {e}")
                total_vouches += get_bonus_vouches(booster.id)["total"]
...
class TicketControlView(discord.ui.View):
    ...
    async def claim_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not has_role:
            allowed_roles = ["als helper", "sp helper", "av helper", "ag helper", "ae helper"]
...
class ParadoxTicketView(discord.ui.View):
    def __init__(self):
        ...
        options = [
            ...
            discord.SelectOption(label="Anime Last Stand (ALS)", emoji=Emojis.GAME, value="ALS"),
    ...
    async def select_callback(self, interaction: discord.Interaction):
        try:
            ...
            game_image_names = {
                ...
                "AV": "av.png",
                "SP": "sp.png",
                "AE": "G2QVvk8O_400x400.jpg",
            }
            game_image_name = game_image_names.get(game_id)
            game_image = get_asset_path(game_image_name) if game_image_name else None
            if game_image and os.path.exists(game_image):
                image_filename = f"game_{game_image_name}"
                game_file = discord.File(game_image, filename=image_filename)
                embed.set_image(url=f"attachment://{image_filename}")
                files.append(game_file)
            elif game_image_name:
                github_image_url = (
                    "https://raw.githubusercontent.com/vr0tex/hopefully-works-now/main/"
                    f"{game_image_name}"
                )
                embed.set_image(url=github_image_url)
...
class HelperApplicationView(discord.ui.View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if game_id in ["AG", "ALS", "SP", "AV", "UTD", "AE"]:
        ...
        try:
            ...
            try:
                if applicant:
                    ...
                    if game_name == "Anime Vanguards (AV)":
                        await applicant.send("you got the av role")
                    elif game_name == "Anime Guardians (AG)":
                        await applicant.send("you got the ag role")
                    elif game_name == "Sailor Piece (SP)":
                        await applicant.send("you got the sp role")
            except Exception as e:
...
async def start_application(user: discord.Member, game_id: str, game_name: str):
    ...
    if game_id == "AG":
        questions = [
            {"text": "1. What is your Roblox username?\n(i.e. 1Nokawa)", "type": "text"},
            {"text": "2. What is your timezone?\n(i.e. GMT+8)", "type": "text"},
            {"text": "3. How active are you on Discord daily?", "type": "text"},
            {"text": "4. Are you able to do 1000% for all types of gamemode?", "type": "yesno"},
            {"text": "5. Are you able to solo new event gamemode?", "type": "yesno"},
            {"text": "6. Picture of the meta units with ultima (ss of your units)", "type": "image"},
            {"text": "7. What type of artifacts you got (ss of them)", "type": "image"},
        ]
    elif game_id == "ALS":
        questions = [
            {"text": "1. What is your Roblox username?", "type": "text"},
            {"text": "2. What is your current Anime Last Stand level?", "type": "text"},
            {"text": "3. What content can you carry in Anime Last Stand?", "type": "text"},
            {"text": "4. How active can you be daily on Discord?", "type": "text"},
            {"text": "5. Can you provide screenshots of your strongest units?", "type": "image"},
        ]
    elif game_id == "SP":
        questions = [
            {"text": "1. How active are you able to be everyday", "type": "text"},
            {"text": "2. Can you solo all Content? (If you cant solo something what is it)", "type": "text"},
            {"text": "3. What melee and sword are you currently using?", "type": "text"},
            {"text": "4. Are you able to give information to help people progress?", "type": "yesno"},
            {"text": "5. What is the best luck build currently?", "type": "text"},
            {"text": "6. What is the best sword currently?", "type": "text"},
            {"text": "7. What is your current Damage percentage? (Provide a screenshot)", "type": "image"},
        ]
    elif game_id == "AV":
        questions = [
        ...
        questions = [
            {"text": "1. What is your Anime Expeditions player level?", "type": "text"},
            {"text": "2. What content can you carry in Anime Expeditions?", "type": "text"},
            {"text": "3. How active can you be daily on Discord?", "type": "text"},
            {"text": "4. Can you provide screenshots of your strongest units?", "type": "image"},
        ]
    else:
        return
            ...
            try:
                while True:
                    ...
                    if msg.content.lower().startswith("!end"):
                        ...
                        return
                    if msg.attachments:
                        answers[i+1] = msg.attachments[0].url
                        break
                    else:
                        await user.send("❌ Please upload an image or type '!end' to cancel.")
            except asyncio.TimeoutError:
                await user.send("❌ Application timed out.")
                return
...
class ParadoxBot(commands.Bot):
    def __init__(self):
        ...
        # Disable presences if tracking isn't needed
...
async def setup(ctx):
    embed = V2Embed()
    file = None
    setup_header_path = get_asset_path("setup_header.png")
...
async def vouch(ctx, target: discord.Member, game: str, *, feedback: str = "Fast, safe, and professional!"):
    ...
    total_vouches += get_bonus_vouches(target.id)["total"]
    embed = create_vouch_embed(ctx.author, target, game, feedback, total_vouches, ctx.channel.id)
    if supabase and db_success:
        await ctx.send(embed=embed)
    else:
        await ctx.send(content="⚠️ Database not connected or offline (Bonus/Local vouches only)", embed=embed)
...
async def givevouch(ctx, target: discord.Member, amount: int, game: str = "ALS"):
    ...
    try:
        ...
        # Get real vouches + bonus vouches to display on card
        total_vouches = get_bonus_vouches(target.id)["total"]
        if supabase:
            try:
                res = supabase.table("vouches").select("id", count="exact").eq("booster_id", str(target.id)).execute()
                ...
                print(f"Error querying Supabase in givevouch: {e}")
    ...
    try:
        ...
        if supabase:
            try:
                ...
                print(f"Error querying Supabase in removevouches: {e}")
...
async def searchup(ctx, member: discord.Member = None):
    ...
    total_vouches = 0
    bonus = get_bonus_vouches(member.id)
    total_vouches += bonus["total"]
    if supabase:
        try:
            res = supabase.table("vouches").select("id", count="exact").eq("booster_id", str(member.id)).execute()
            total_vouches += (res.count if res.count is not None else 0)
        except Exception:
            pass
