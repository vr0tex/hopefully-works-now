...
def load_welcome_settings():
    try:
        ...
        with open(WELCOME_SETTINGS_FILE, "r", encoding="utf-8") as fh:
            data = json.load(fh)
        return {int(guild_id): bool(enabled) for guild_id, enabled in (data or {}).items()}
...
def save_welcome_settings():
    try:
        with open(WELCOME_SETTINGS_FILE, "w", encoding="utf-8") as fh:
            json.dump({str(guild_id): bool(enabled) for guild_id, enabled in WELCOME_ENABLED.items()}, fh)
    except Exception as exc:
...
def get_total_vouches(user_id, minimum_total=0):
    ...
    try:
        ...
        return max(total_vouches + database_total, minimum_total)
    except Exception as e:
        print(f"Error querying total vouches: {e}")
        return max(total_vouches, minimum_total)
...
def add_bonus_vouches(user_id, amount, game):
    ...
    with open(os.path.join(BASE_DIR, "bonus_vouches.json"), "w") as f:
        json.dump(data, f)
...
class VouchModal(discord.ui.Modal, title="Vouch Feedback"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if VOUCH_CHANNEL_ID != 0:
            ...
            if vouch_channel:
                minimum_total = previous_total + 1 if vouch_saved else previous_total
...
async def welcome(ctx, setting: str = None):
    ...
    if value in ("on", "enable", "enabled", "true"):
        ...
        await ctx.send(f"✅ Welcome system turned **on**. New joins will be posted in <#{WELCOME_CHANNEL_ID}>.")
    elif value in ("off", "disable", "disabled", "false"):
        WELCOME_ENABLED[guild_id] = False
        save_welcome_settings()
        await ctx.send(f"✅ Welcome system turned **off**. No welcome posts will be sent in <#{WELCOME_CHANNEL_ID}>.")
    else:
...
async def testwelcome(ctx):
    ...
    embed = V2Embed(
        title="Welcome to Hypersons",
        description=(
            f"**Welcome, {ctx.author.mention}!**\n\n"
            "We’re glad you’re here.\n"
            "Make sure to read the server rules and jump into the community!"
        ),
        color=discord.Color.blurple()
    )
    embed.add_field(name="Server Guides", value="Check the rules, ticket channels, and helper applications in the server.", inline=False)
    if file:
        embed.set_image(url="attachment://welcome_banner.png")
...
async def myvouches(ctx):
    ...
    if supabase:
        try:
            ...
            latest_vouch = vouch_list[0] if vouch_list else None
            if latest_vouch:
